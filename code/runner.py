...
class Runner:
    ...
    def load_x_train(self) -> pd.DataFrame:
        ...
        return pd.read_csv('../input/train.csv').drop(['arousal_mean','valence_mean'], axis=1)
    ...
    def load_y_train(self) -> pd.Series:
        ...
        """
        # 目的変数の読込を行う
        train_y = pd.read_csv('../input/train.csv')['valence_mean']
    ...
    def load_x_test(self) -> pd.DataFrame:
        ...
        :return: テストデータの特徴量
        """
        return pd.read_csv('../input/test.csv').drop(['arousal_mean','valence_mean'], axis = 1)
